        ...
        series: [
            {
                ...
                // 图表类型
                type: "pie",
                // 饼形图半径，可以是px也可以是百分比(基于DOM容器大小)
                // 如果radius是百分比则必须加引号
                // 这里用两个值是因为南丁格尔图有两个圆，内圆半径和外圆半径
                radius: ["10%", "70%"],
                // 图标中心位置，距离图表DOM容器
                center: ["50%", "50%"],
                // redius的半径模式，另外一种是area面积模式
                roseType: "radius",
                // value数据值，name数据名称
                data: [
                    { value: 20, name: "云南" },
...
(function () {
    ...
    // 声明一个函数里面设置sup当前li高亮还有对应的品牌对象渲染
    // 这个函数需要传递当前元素进去
    function render(currentEle) {
        ...
        // 我们可以通过hotData[$(this).index()].brands拿到的是城市对象的品牌种类
        // 开始遍历品牌数组
        var subHTML = "";
